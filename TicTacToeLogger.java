 ...
 *  Two-player game of Tic-Tac-Toe. This version logs the output 
 *  board into a text file named 'log.txt'.
 *
...
public class TicTacToeLogger { 
   ...
   public static void main(String[] args) throws IOException {
      ...
      String out = "";
      fileByteStream = new FileOutputStream("log.txt", false);
      out1 = new PrintStream(fileByteStream);
   
      while (status == IN_PROGRESS) {
         player = switchPlayer(player);
         displayBoard(markerForPlayer, board, out1);
   ...
   public static void displayBoard(char[] markerForPlayer, char[] board, 
         PrintStream out) {
      displayHeader(markerForPlayer, out);
      for (int row = 0; row < SIZE; row++) {
         displayBoardRowSeparator(out);
         displayBoardRow(board, row, out);
      }
      displayBoardRowSeparator(out);
   }
    ...
    * @param out PrintStream object
    */
   public static void displayBoardRow(char[] board, int row, PrintStream out) {
      int index;
      for (int col = 0; col < SIZE; col++) {
         index = getLinearIndex(row, col);
         System.out.print("|");
    ...
    *  @param out PrintStream object
    */
   public static void displayBoardRowSeparator(PrintStream out) {
